...
- For `model_type = "s_learner"` or `"m_learner"`, what `model_params` sets as `NULL` falls back to depends on the following learner types:

  - For `learner_type = "ridge"`,  the method defaults to `glmnet::cv.glmnet()` with `alpha = 1`, corresponding to Ridge regression.
  ...
  ```
  - For `learner_type = "caret"`, it defaults to a linear regression:
  ```r
  ...
  ```
  However, even if `Y` is binary, if you are using `model_type = "m_learner"`, you need to use a regression method per definition of the M learner which re-defines the target as a transformed `Y` (see the Cram method paper given as reference in "Introduction & Cram Policy" for more details). 
