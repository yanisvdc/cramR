...
The `cram_ml()` function implements the CRAM method — **Causal Regularization via Approximate Models** — for **simultaneous model training and evaluation**, even when the data-generating process is unknown.

It supports:

- Supervised learning via formulas (e.g., `Y ~ .`)
- Unsupervised learning (e.g., `kmeans`)
- Batch-wise model training and evaluation
- Standard or custom loss functions
- Confidence intervals for expected loss
...
- `loss_name`: built-in options like `"mse"`, `"logloss"`, `"accuracy"`, `"euclidean_distance"`  
