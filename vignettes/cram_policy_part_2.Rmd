...
In the article "Introduction & Cram Policy part 1", we presented the Cram method to simultaneously learn and evaluate a binary policy. We discussed the main parameters of cram_policy and showed how to run the function on an example dataset. In the following section, we provide an in-depth explanation on how to set the parameters for different use cases.
...
Please note that the list should contain an element `formula` where `Y` refers to the vector that you provided as input for `Y`, and an element named `caret_params` containing the parameters of your choice to pass to `caret::train` (see full list of parameters here: https://topepo.github.io/caret/model-training-and-tuning.html).
  ...
  - outcome_transform is an element of model_params accessed as follows: `outcome_transform <- model_params$m_learner_outcome_transform`. If `NULL`, it defaults to `outcome_transform <- function(Y, D, prop_score) {Y * D / prop_score - Y * (1 - D) / (1 - prop_score)}`, where `prop_score` is `propensity(X)`. Thus, the Y transformed might not be categorical even though Y is categorical i.e. you might want to use a regression model (see the following reference for more details about the M-learner: Jia, Z., Imai, K., & Li, M. L. (2024). *The Cram Method for Efficient Simultaneous Learning and Evaluation*, URL: https://www.hbs.edu/ris/Publication%20Files/2403.07031v1_a83462e0-145b-4675-99d5-9754aa65d786.pdf.). 
